...
class CommentsBloc extends ChangeNotifier{
  ...
  Future<List> getData(timestamp, types) async {
    ...
    QuerySnapshot snap = await FirebaseFirestore.instance.collection('$types/$timestamp/comments').get();
    var x = snap.docs;
    List data = [];
  ...
  Future saveNewComment(timestamp,types, comment)async{
    ...
    await _getDate().then((_){
     FirebaseFirestore.instance.collection('$types/$timestamp/comments').doc('$_uid$timestamp1').set({
        'name': _name,
        'comment' : comment,
        'date' : date,
        'image url' : _imageUrl,
  ...
  Future deleteComment (timestamp, types, uid, timestamp2) async{
   ...
   FirebaseFirestore.instance.collection('$types/$timestamp/comments').doc('$uid$timestamp2').delete();
    getData(timestamp, types);
  }
