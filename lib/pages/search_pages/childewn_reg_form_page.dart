...
class ChildrenRegisterDilougPage extends StatefulWidget {
  ...
  final String email;
  const ChildrenRegisterDilougPage(
    this.timestamp,
    this.terms,
    this.childreNumber,
    this.function,
...
class _RegisterDilougState extends State<ChildrenRegisterDilougPage> {
  ...
  Future<String> saveSig(SignatureController controller, String uid) async {
  try{
      Uint8List image = await  controller.toPngBytes();
   TaskSnapshot  tp = await FirebaseStorage.instance.ref().child('signitures')
        .child(uid + ".jpg")
        .putData(image);
 return await tp.ref.getDownloadURL();
  }catch(e){
    log(e.toString());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: Text(
              'register now'
                  .tr() /*+ " " +  'places are available'.tr() + " " + widget.childreNumber.toString()*/,
                    ...
                    child: ListView.separated(
                        ...
                        itemBuilder: (_, index) {
                            ...
                            return GestureDetector(
                              ...
                              child: CircleAvatar(
                                ...
                                radius:
                                    MediaQuery.of(context).size.height * 0.06,
                                child: Text(
                                  textFiledsControllers[index - 1].text,
                          ...
                          return GestureDetector(
                              ...
                              child: CircleAvatar(
                                backgroundImage: FileImage(files[index - 1]),
                                radius:
                                    MediaQuery.of(context).size.height * 0.06,
                              ));
                        }),
  ...
  Column childData() {
    return Column(
      children: [
        TextFormField(
          decoration: inputDecoration('child name'.tr(), 'child'.tr(),
              textFiledsControllers[currentIndex]),
        ...
        TextFormField(
          decoration: inputDecoration('child age'.tr(), 'age'.tr(),
              textFiledsControllersAges[currentIndex]),
  ...
  Column parentData() {
    return Column(
      children: [
        ...
        TextFormField(
          decoration:
              inputDecoration('parent name'.tr(), 'name'.tr(), parentCtrl),
        ...
        TextFormField(
          decoration: inputDecoration(
              'national id'.tr(), 'national id'.tr(), parentCtrl),
  ...
  Future sendEmail() async{
    if (/*files.length != 0 &&*/ _controller
                                              .isNotEmpty &&
                                          isComplete()) {
                                        setState(() {
                                         openToast(context, "uploading");
                                        });
                                        List children = [];
                                        for (int i = 0;
                                            i < textFiledsControllers.length;
                                            i++) {
                                          children.add({
                                            "name":
                                                textFiledsControllers[i].text,
                                            "age": textFiledsControllersAges[i]
                                                .text,
                                          });
                                        }
                                        String sigLink = await saveSig(_controller, ub.getUid);
                                    
                                        final Email email = Email(
                                          subject:
                                              'Registration Request - طلب تسجيل',
                                          body: "parent name/ولي الأمر " +
                                              parentCtrl.text +
                                              "\n" +
                                              " national id/الرقم المدني" +
                                              natIDCtrl.text +
                                              "\n" +
                                              " appotment date/ تاريخ المقابلة " +
                                              dateTime +
                                              "\n" +
                                              " children/ الأطفال " +
                                              children
  ...
  bool isComplete() {
    ...
    return parentCtrl.text.isNotEmpty && natIDCtrl.text.isNotEmpty &&dateTime.isNotEmpty;
  }
}
