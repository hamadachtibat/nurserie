...
class CommentsPage extends StatefulWidget {
  ...
  const CommentsPage({Key key, @required this.title, @required this.timestamp}) : super(key: key);
...
class _CommentsPageState extends State<CommentsPage> {
  ...
  handleDelete (uid, timestamp2) async {
    ...
    final CommentsBloc cb = Provider.of<CommentsBloc>(context ,listen: false);
    showDialog(
      context: context,
      builder: (BuildContext context){
        return AlertDialog(
          title: Text('Delete?'.tr(), style: TextStyle(
            fontWeight: FontWeight.w600
          ),),
          content: Text('Do you Want to Delete This Comment?'.tr()),
          actions: <Widget>[
            FlatButton(
              child: Text('yes'.tr()),
              onPressed: () async {
                ...
                if(ib.hasInternet == false){
                  openToast(context, 'No internet connection'.tr());
                } else{
                  final SharedPreferences sp = await SharedPreferences.getInstance();
                  String _uid = sp.getString('uid');
                  if(uid != _uid){
                    openToast(context, "You can't delete others comments".tr());
                  }else{
                    await cb.deleteComment(widget.timestamp, 'places', uid, timestamp2);
                     openToast(context, 'Deleted Successfully'.tr());
                    ...
                    /*if(widget.title == 'User Reviews'){
                     ...
                     await cb.deleteComment(widget.timestamp, 'blogs', uid, timestamp2);
                     openToast(context, 'Deleted Successfully');
    ...
    return Scaffold(
      ...
      body: Column(
        children: <Widget>[
          Expanded(
              child: FutureBuilder(
              future: cb.getData(widget.timestamp, 'places'),  /*widget.title == 'User Reviews' ? cb.getData(widget.timestamp, 'places') : cb.getData(widget.timestamp,'blogs')*/
              builder: (BuildContext context, AsyncSnapshot<List> snap){

                if(snap.connectionState == ConnectionState.none) return emptyPage(Icons.signal_wifi_off, 'No Internet'.tr());
                else{
                  if(!snap.hasData) return Center(child: CircularProgressIndicator());
                  if(snap.hasError) return emptyPage(Icons.error_outline, 'something wrong!'.tr());
                  if(snap.data.isEmpty) return emptyPage(Icons.mode_comment, 'no comments are found'.tr());
                  ...
                  child: ListTile(
                    leading: CircleAvatar(
                      ...
                      title: Row(
                            ...
                            children: <Widget>[
                              Text(d[index]['name'], style: TextStyle(
                                color: Colors.black,
                                fontSize: 12,
                      ...
                      subtitle: Text(d[index]['comment'], style: TextStyle(
                        fontSize: 15,
